...
app.config["DATA_COLLECTOR_URL"] = os.environ.get(
    "DATA_COLLECTOR_URL", "http://data-collector:5001"
)
app.config["ALERT_SERVICE_URL"] = os.environ.get(
    "ALERT_SERVICE_URL", "http://alert-service:5002"
)
app.config["POLICY_SERVICE_URL"] = os.environ.get(
    "POLICY_SERVICE_URL", "http://policy-orchestrator:5004"
)
app.config["COMPLIANCE_ENGINE_URL"] = os.environ.get(
    "COMPLIANCE_ENGINE_URL", "http://compliance-engine:5007"
)
app.config["XAI_SERVICE_URL"] = os.environ.get(
    "XAI_SERVICE_URL", "http://xai-service:5006"
)
app.config["AI_ENGINE_URL"] = os.environ.get("AI_ENGINE_URL", "http://ai-engine:5003")
app.config["DRL_ENGINE_URL"] = os.environ.get(
    "DRL_ENGINE_URL", "http://drl-engine:5005"
)
app.config["HARDENING_SERVICE_URL"] = os.environ.get(
    "HARDENING_SERVICE_URL", "http://hardening-service:5011"
)
app.config["HIDS_AGENT_URL"] = os.environ.get(
    "HIDS_AGENT_URL", "http://hids-agent:5010"
)
...
def get_request_stats():
    """Get API request statistics from the last 5 minutes (efficient scan)."""
...
def _fetch_downstream_stats():
    ...
    try:
        resp = requests.get(
            f"{app.config['ALERT_SERVICE_URL']}/api/v1/alerts/statistics",
            headers=headers,
            timeout=3,
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            result["threats_detected"] = resp.json().get("total", 0)
    except requests.exceptions.RequestException:
        logger.debug("Data collector stats unavailable")

    try:
        resp = requests.get(
            f"{app.config['POLICY_SERVICE_URL']}/api/v1/statistics",
            headers=headers,
            timeout=3,
        )
        if resp.status_code == 200:
...
def health_check():
    ...
    return jsonify(
        {"status": "healthy", "timestamp": time.time(), "request_stats": stats}
    ), 200
...
def resolve_alert(alert_id):
    """Resolve an alert"""
    return _proxy_to(
        app.config["ALERT_SERVICE_URL"], f"/api/v1/alerts/{alert_id}/resolve"
    )
...
def update_config():
    ...
    try:
        ...
        logger.info(
            f"Configuration updated by {g.current_user['username']}: {json.dumps(new_config)}"
        )
...
def get_statistics():
    ...
    try:
        downstream = _fetch_downstream_stats()
        stats = {
...
def _sse_pubsub_stream(channel: str, heartbeat_type: str):
    """Subscribe to a Redis pub/sub channel and yield SSE frames.

    Emits real events as they arrive and sends a heartbeat every 15 s
    to keep the connection alive through proxies/load balancers.
    """
    pubsub_client = redis.from_url(app.config["REDIS_URL"], decode_responses=True)
...
def stream_threats():
    ...
    return Response(
        _sse_pubsub_stream(SSE_CHANNEL_THREATS, "threat_heartbeat"),
...
def stream_alerts():
    ...
    return Response(
        _sse_pubsub_stream(SSE_CHANNEL_ALERTS, "alert_heartbeat"),
        mimetype="text/event-stream",
        headers={"Cache-Control": "no-cache", "X-Accel-Buffering": "no"},
    )
...
def test_rate_limit():
    ...
    return jsonify(
        {"message": "Rate limit test successful", "timestamp": time.time()}
    ), 200
...
def _proxy_to(base_url, path_suffix, methods=None):
    ...
    try:
        if request.method == "GET":
            ...
            resp = requests.put(
                url, headers=headers, json=request.get_json(silent=True), timeout=30
            )
        elif request.method == "DELETE":
...
def get_framework(framework_id):
    return _proxy_to(
        app.config["COMPLIANCE_ENGINE_URL"], f"/api/v1/frameworks/{framework_id}"
    )
...
def hardening_remediations():
    ...
    return _proxy_to(
        app.config["HARDENING_SERVICE_URL"], "/api/v1/hardening/remediations"
    )
...
def hardening_remediate(check_id):
    ...
    return _proxy_to(
        app.config["HARDENING_SERVICE_URL"], f"/api/v1/hardening/remediate/{check_id}"
    )
...
def audit_categories():
    ...
    return jsonify({"categories": [c.value for c in AuditCategory]}), 200
