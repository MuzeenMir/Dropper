...
def _fetch_downstream_stats():
    ...
    headers = {'Authorization': f'Bearer {os.environ.get("INTERNAL_SERVICE_TOKEN", "")}'}
    ...
    try:
        resp = requests.get(
            ...
            headers=headers, params={'limit': 1}, timeout=3,
        )
        if resp.status_code == 200:
    ...
    try:
        while True:
            ...
            if msg and msg['type'] == 'message':
                ...
                heartbeat = json.dumps({'type': heartbeat_type, 'timestamp': time.time()})
