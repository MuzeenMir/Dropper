# Dropper — Watch-test bundle (v0.1.1)

This is a pre-release build of Dropper handed to you for a 7-day watch test.
It is **not** a finished product. Read this once before running anything.

## What Dropper does

Dropper runs as a local DNS resolver on your Windows machine. When your
browser, email client, or any other program tries to look up a domain name
(say `example.com`), Dropper answers — and if that domain is on a public
list of known-malicious sites (URLhaus), Dropper returns its own address
...
If the domain is benign, Dropper asks an upstream resolver (Cloudflare or
...
- **No tray icon.** Dropper runs hidden in the background. The
  `dropper.log` and `dropper.err.log` files in this folder are the
...
- **Auto-start across reboots.** `setup.ps1` registers a Task Scheduler
  task (`DropperWatchtest`) that re-launches dropper.exe every time
...
| File                  | Purpose                                          |
|-----------------------|--------------------------------------------------|
| `dropper.exe`        | The actual program. Cross-compiled for Windows.  |
| `setup.ps1`           | Install: backs up DNS, swaps to 127.0.0.1, runs dropper.exe. |
| `uninstall.ps1`       | Revert: stops dropper.exe, restores DNS, flushes cache. |
...
Source code: https://github.com/MuzeenMir/Dropper
License: MIT
Issues: https://github.com/MuzeenMir/Dropper/issues
